...
import { BUILDINGS_URL_API } from "../../Urls"

export function ShowPossibleBuildings({setPayload, company, currentBuilding }: {
    company: any,
    currentBuilding: any,
    ...
    const [lastCompany, setCompany] = useState('')
    ...
    if(currentUrl === '' || company.id != lastCompany) {
        setCompany(company.id)
        setCurrentUrl(BUILDINGS_URL_API(company.id))
    }
    ...
    return (
        <div className="space-y-1 p-3 bg-white rounded-lg border border-gray-200">
            <p>Selected building: {currentBuilding?.name}</p>
            ...
            <CollectionPagination collection={getPropertiesOrUndefined(result?.body)} setUrlFunction={setCurrentUrl} 
                templateUrl={getLink('pagination', result?.body)?.href.replace('{companyId}', lastCompany)}/>
            <Outlet/>
        </div>
    )
}